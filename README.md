...
`weaver.pure.traced` supports test tracing via `otel4s`. The example uses the honeycomb backend.

Current limitations:

- Only tracing in `IO` is supported
- Only `jvm` backend is supported.
